...
1. 使用以下命令启动容器。
   ...
   docker run -it --name moocake_patch --privileged=true --net=host --shm-size=128g <commit ID> bash
   # 需要替换基础镜像的commit ID
...
5. 使用以下命令保存镜像。
   ...
   docker commit -a "add Mooncake Conductor" moocake_patch mindie-motor-vllm:dev-26.0.0.B060-800I-A3-py311-Ubuntu24.04-lts-aarch64-patch
...
开启KV Cache亲和性调度能力前请参考[pyMotor快速开始](../../../README.md)，确保环境能正常完成基础的服务部署。

### 配置user_config.json

参考[vllm kv_events文档](https://docs.vllm.ai/en/stable/api/vllm/config/kv_events/)中kv-events-config配置，在user_config.json配置文件中需要在P实例中增加kv-events-config配置，以[pyMotor快速开始](../../../README.md)中实例uesr_config.json为参考基线，适配打开KV Cache亲和性调度能力的配置文件示例如下
