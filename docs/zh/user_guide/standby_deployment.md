...
1. 配置Coordinator侧证书挂载。
    ...
    <b>如果不开启CA证书，请跳过此步骤。</b>
    如果需要开启证书CA认证，根据[生成etcd安全证书（可选）](#生成etcd安全证书可选)生成的证书文件，将证书文件的生成路径挂载至Coordinator容器内。在 `examples/deployer/yaml_template/coordinator_template.yaml` 文件中的 volumeMounts 和 volumes 中添加如下内容（coordinator-ca 为挂载的证书目录）：
...
2. 在集群中创建ETCD资源。
    ...
    4. 执行以下命令自行创建etcd.yaml文件，配置ETCD Pod侧证书。
        ...
        根据[3.1](standby_deployment.md#生成etcd安全证书可选)生成的证书，将文件生成路径挂载至ETCD容器内，并配置ETCD使用加密通信，指定使用ca.pem、server.pem和server.key进行通信。
