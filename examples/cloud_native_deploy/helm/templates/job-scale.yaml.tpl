{{- if and .Values.jobs.scale.enabled (eq .Values.operation "scale") }}
apiVersion: batch/v1
kind: Job
metadata:
  name: {{ include "mindie-pymotor.jobName" (dict "root" . "name" .Values.jobs.scale.name) }}
  labels:
    ...
    spec:
      ...
      containers:
        - name: scaler
          ...
          command: ["/app/examples/cloud_native_deploy/entrypoint-scale.sh"]
          env:
            - name: CONFIG_DIR
