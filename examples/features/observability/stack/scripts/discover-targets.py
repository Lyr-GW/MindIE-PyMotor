...
def _discover(namespace: str, node_ip: str, args: argparse.Namespace) -> DiscoveryResult:
    ...
    if _is_kubectl_ready():
        try:
            ...
            controller_match = _find_service_nodeport(
                services=services,
                expected_port=DEFAULT_CONTROLLER_PORT,
                service_keywords=("controller", "mindie-motor-controller"),
                port_keywords=("metrics", "observ", "obs"),
            )
            if controller_match:
                _, controller_port = controller_match
            else:
                warnings.append(
                    f"controller NodePort not found, fallback to default {node_ip}:{DEFAULT_CONTROLLER_PORT}."
                )
    ...
    controller_metrics_url = (
        f"http://{controller_host}:{controller_port}/observability/metrics"
    )
...
def _build_prometheus_config(result: DiscoveryResult) -> str:
    ...
    controller_targets = [
        {
            "target": "controller-metrics-proxy:9106",
            "labels": {
    ...
    lines.append("")
    lines.extend(_render_job("motor-controller", controller_targets, metrics_path="/metrics"))
    lines.append("")
...
def _build_env(result: DiscoveryResult) -> str:
    ...
    lines = [
        ...
        f"CONTROLLER_METRICS_URL={result.controller_metrics_url}",
        ...
        f"CONTROLLER_PROXY_PORT={os.getenv('CONTROLLER_PROXY_PORT', str(DEFAULT_CONTROLLER_PROXY_PORT))}",
    ]
    return "\n".join(lines) + "\n"
...
def _build_summary(result: DiscoveryResult) -> str:
    lines = [
        ...
        f"Controller URL: {result.controller_metrics_url}",
