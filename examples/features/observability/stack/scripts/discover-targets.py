...
def _resolve_port(env_key: str, default: int) -> int:
...
def _build_env(result: DiscoveryResult) -> str:
    ...
    otlp_http_port = _resolve_port("OTEL_HTTP_PORT", DEFAULT_OTLP_HTTP_PORT)
    otlp_grpc_port = _resolve_port("OTEL_GRPC_PORT", DEFAULT_OTLP_GRPC_PORT)
    lines = [
        "# Auto-generated by scripts/discover-targets.py",
        ...
        f"OTLP_HTTP_ENDPOINT=http://{obs_host}:{otlp_http_port}/v1/traces",
        f"OTLP_GRPC_ENDPOINT=http://{obs_host}:{otlp_grpc_port}",
        ...
        f"OTEL_GRPC_PORT={otlp_grpc_port}",
        f"OTEL_HTTP_PORT={otlp_http_port}",
