...
| 看板 | UID | 文件 | 内容 |
|------|-----|------|------|
| **vLLM 指标监控** | `motor-metrics-monitoring` | `motor-metrics-monitoring.json` | 运维总览：服务状态、吞吐与时延、KV Cache、主机和 NPU 资源 |
...
以「vLLM 指标监控」为例，顶部变量用于跨集群 / 角色 / 实例过滤，均为 `query` 类型并基于 Prometheus 标签动态生成：
...
6. 写回源文件以长期保留：点击看板设置（齿轮）→ **JSON Model**，复制完整 JSON，覆盖写入对应文件，例如：
   - vLLM 指标监控 → `grafana/dashboards/motor-metrics-monitoring.json`
...
在 `grafana/dashboards/<dashboard>.json` 的 `panels` 数组中追加一个面板对象。可参考「vLLM 指标监控」中现有 `stat` 面板的最小结构：
...
> vLLM 指标监控中的基础指标无需改 pyMotor 配置即可生效；当前方案不使用 Controller metrics 接口，相关配置可忽略。
