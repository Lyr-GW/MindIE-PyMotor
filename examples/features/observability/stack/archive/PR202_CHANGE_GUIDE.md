...
本文档说明 PR202（MR !202）可观测性一键栈的合入范围、评审要点与验收步骤。日常使用说明见 [SERVICE_GUIDE.md](../SERVICE_GUIDE.md)。**PR #20 联调拉起**见 [PR20_LAUNCH_GUIDE.md](PR20_LAUNCH_GUIDE.md)。
...
- [x] 显式 `MOTOR_NAMESPACE` 时，kubectl 不可达应报错而非静默 fallback
- [x] `vllm-p0` / `vllm-d0` 等 Pod 能被识别为 engine target
- [x] Docker runtime 下 Coordinator / Engine PodIP 生成 `PORT_FORWARD_*` 与 `host.docker.internal` target
- [x] `docker compose up` 默认 `--pull missing`（本地有镜像不拉）、不 build Grafana（`OBS_COMPOSE_BUILD=1` 可显式开启）
- [x] Grafana 容器 `HTTP_PROXY` 为空，`NO_PROXY` 含 `prometheus,tempo`
- [x] 运行时产物（`generated/`、`.env`）未提交

---

## 4. 代理环境拉起

与 [SERVICE_GUIDE.md §2.2](../SERVICE_GUIDE.md#22-代理分工需区分阶段)、[Checklist §5.4](PR202_LAUNCH_FIX_CHECKLIST.md#54-代理分工) 一致：
