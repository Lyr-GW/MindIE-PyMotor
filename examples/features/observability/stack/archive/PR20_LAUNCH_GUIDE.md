...
本文档面向 **PR #20**（`feat(observability): PR202 一键发现并拉起本地观测栈`）的联调与验收，说明前提条件、镜像与代理策略，以及 **`launch.sh` 统一入口** 与各启动模式。日常参数与端口见 [SERVICE_GUIDE.md](../SERVICE_GUIDE.md)；合入范围见 [PR202_CHANGE_GUIDE.md](PR202_CHANGE_GUIDE.md)；联调问题根因见 [PR202_LAUNCH_FIX_CHECKLIST.md](PR202_LAUNCH_FIX_CHECKLIST.md)。
...
- **代理与 pull**：`--pull missing`、Grafana 容器禁代理、kubectl 清代理，见 PR 说明与 [SERVICE_GUIDE.md §2.2](../SERVICE_GUIDE.md#22-代理分工需区分阶段)。
