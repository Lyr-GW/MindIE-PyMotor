...
def test_start_inference_only_starts_inference_only(mock_create_socket):
    """_on_become_master starts only Inference; Mgmt is not started."""
    mock_create_socket.return_value = None
...
@patch("motor.coordinator.daemon.coordinator_daemon.create_shared_socket")
def test_start_processes_does_not_sleep(mock_create_socket):
    """Daemon no longer waits 2s after a Scheduler process (process was removed)."""
    mock_create_socket.return_value = None
    ...
    daemon._start_processes([PROCESS_KEY_MGMT])
...
def test_process_table_has_no_scheduler_process():
    """P3 gate: Daemon process table no longer includes SchedulerProcess."""
    import motor.coordinator.process.constants as constants

    assert not hasattr(constants, "PROCESS_KEY_SCHEDULER")
    assert "SchedulerProcess" not in START_ORDER
    assert "SchedulerProcess" not in STOP_ORDER
    assert START_ORDER == [PROCESS_KEY_MGMT, PROCESS_KEY_OBS, PROCESS_KEY_INFERENCE]
    assert STOP_ORDER == [PROCESS_KEY_INFERENCE, PROCESS_KEY_OBS, PROCESS_KEY_MGMT]
