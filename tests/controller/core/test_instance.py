...
def test_instance_active() -> None:
    ...
    pod_ip = "127.0.0.1"
    endpoints = {
        1: Endpoint(id=1, ip=pod_ip, business_port="1001"),
        2: Endpoint(id=2, ip=pod_ip, business_port="1002"),
    }
    instance = Instance(
...
def test_add_endpoints() -> None:
    ...
    pod_ip1 = "127.0.0.1"
    endpoints1 = {
        1: Endpoint(id=1, ip=pod_ip1, business_port="1001"),
        2: Endpoint(id=2, ip=pod_ip1, business_port="1002"),
    }
    instance = Instance(
...
def test_is_endpoints_enough_greater_than_dp_size() -> None:
    ...
    endpoints = {
        1: Endpoint(id=1, ip=pod_ip, business_port="1001"),
        2: Endpoint(id=2, ip=pod_ip, business_port="1002"),
        3: Endpoint(id=3, ip=pod_ip, business_port="1003"),  # Extra endpoint
    }
    instance = Instance(
...
def test_get_all_endpoints_filters_headless_multi_endpoints_disabled() -> None:
    ...
    instance.add_endpoints(
        "10.0.0.2",
        {0: Endpoint(id=0, ip="10.0.0.2", business_port="8000", headless=True)},
    )
    endpoints = instance.get_all_endpoints()
    assert len(endpoints) == 1
    assert endpoints[0].ip == "10.0.0.1"
...
def test_get_all_endpoints_single_pod_all_headless() -> None:
    ...
    instance.add_endpoints(
        "10.0.0.1",
        {0: Endpoint(id=0, ip="10.0.0.1", business_port="8000", headless=True)},
    )
    endpoints = instance.get_all_endpoints()
...
def test_endpoint_headless_defaults_to_false() -> None:
    ...
    endpoint = Endpoint(id=0, ip="10.0.0.1", business_port="8000")
    assert endpoint.headless is False


@pytest.mark.parametrize("extra_field", ["mgmt_port", "headles", "bootstrap_prt"])
def test_endpoint_rejects_unknown_fields(extra_field: str) -> None:
    """Unknown endpoint fields must fail instead of being silently discarded."""
    payload = {"id": 0, "ip": "10.0.0.1", "business_port": "8000", extra_field: "9000"}
...
def test_is_any_endpoint_paused() -> None:
    ...
    instance.add_endpoints(
        "10.0.0.1",
        {0: Endpoint(id=0, ip="10.0.0.1", business_port="8000", status=EndpointStatus.NORMAL)},
    )
    instance.add_endpoints(
        "10.0.0.2",
        {0: Endpoint(id=1, ip="10.0.0.2", business_port="8000", status=EndpointStatus.PAUSED)},
    )
