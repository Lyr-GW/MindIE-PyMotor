...
"""800I-A2 / 800I-A3 store bootstrap: ACL context only, deliberately no HCCL comm.

A comm would make the store's adxl engine merge its root info into the P2P
handshake; on same-node PD the store shares devices with the vllm workers, the
merged rank table repeats a device IP and HCCL rejects it with EI0014. Comm-free
handshake is TP-agnostic. ACL context is still required (the store never calls
``aclrtSetDevice`` itself).
"""
