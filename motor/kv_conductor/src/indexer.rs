...
/// to evict expired entries from the pending pool cache.
...
/// at any time.  Once both sides have arrived, the entry is removed from
/// both maps and inserted into the radix tree (or flat store).
///
...
pub(crate) struct OffloadPoolState {
    /// `block_hash → tokens_hash`: offload events waiting for pool confirmation.
    pub(crate) offload: FxHashMap<u64, u64>,
...
impl IndexerEntry {
    ...
    ///
    /// For each `(block_hash, tokens_hash)` pair, checks whether there are
    /// pending pool backend events waiting for this block.  Matched entries
    /// are removed from `pending_pool` and returned (grouped by worker) so
    /// the caller can insert them into the radix tree / flat store.
    /// Unmatched entries are cached in `offload` for later pool confirmation.
    pub fn ingest_offload_blocks(
        &self,
        pairs: &[(u64, u64)],
    ) -> HashMap<WorkerKey, Vec<KvCacheStoredBlockData>> {
        let matched = {
            let mut state = self.offload_pool_state.write();
            let mut matched: HashMap<WorkerKey, Vec<KvCacheStoredBlockData>> = HashMap::new();
                ...
                if let Some(pending) = state.pending_pool.remove(&block_hash) {
                    for entry in pending {
                        matched
                            .entry(entry.worker)
                            .or_default()
                            .push(KvCacheStoredBlockData {
                                block_hash,
                                tokens_hash,
                            });
                    }
                } else {
                    state.offload.insert(block_hash, tokens_hash);
                }
            }
            matched
    ...
    /// Ingest pool backend blocks from Mooncake stored events.
    ///
    /// For each `block_hash`, checks the `offload` cache for a pre-computed
    /// `tokens_hash`.  Matched entries are removed from `offload` and returned
    /// as `KvCacheStoredBlockData` blocks for radix tree insertion.
    /// Unmatched entries are queued in `pending_pool` so that a future
    /// offload event can complete the match.
    pub fn ingest_pool_blocks(
        ...
        let matched = {
            ...
            for &bh in block_hashes {
                if let Some(tokens_hash) = state.offload.remove(&bh) {
                    matched.push(KvCacheStoredBlockData {
                        block_hash: bh,
                        tokens_hash,
                    });
                } else {
    ...
    /// Evict blocks from both caches (for removal events).
    ///
    /// Returns `block_hashes` that were in **neither** cache — these were
    /// already matched and inserted into the tree, so the caller must apply
    /// tree removal for them.
    ///
    /// Blocks found in either cache were still pending and are simply evicted
    /// (they never entered the tree).  This also cleans up `pending_pool`
    /// entries for the specific `worker`.
    pub fn evict_pending_blocks(&self, block_hashes: &[u64], worker: &WorkerKey) -> Vec<u64> {
        let mut state = self.offload_pool_state.write();
        let mut need_tree_removal = Vec::new();

        for &bh in block_hashes {
            // Try offload cache first
            if state.offload.remove(&bh).is_some() {
                // Was waiting for pool event — now cancelled
                continue;
            }
            // Try pending pool events
            ...
            // Not in either cache — must already be in the tree
    ...
    /// block hashes that were affected.
    ///
    /// Note: the `offload` map has no per-worker association, so offload
    /// entries are not purged here — they are bounded by the number of
    /// outstanding offloaded blocks and evicted on match or TTL sweep.
    ...
    /// Total number of pending entries across both caches (for diagnostics).
    pub fn pending_count(&self) -> usize {
        let state = self.offload_pool_state.read();
        state.offload.len() + state.pending_pool.len()
    }

    /// Sweep stale `pending_pool` entries that exceed `ttl`, returning the
    /// number evicted.
    ///
    /// Only `pending_pool` entries are swept (they carry per-entry timestamps).
    /// `offload` entries have no timestamp and are kept until matched by a
    /// pool event or explicitly evicted by a removal event — they are bounded
    /// by the number of outstanding offloaded blocks.
    pub fn sweep_stale_pending(&self, ttl: std::time::Duration) -> usize {
        let mut state = self.offload_pool_state.write();
        ...
        state.pending_pool.retain(|_bh, entries| {
            entries.retain(|e| {
                let keep = now.duration_since(e.inserted_at) < ttl;
                if !keep {
                    pruned += 1;
                }
                keep
    ...
    /// Periodically sweep stale pending pool entries when the ingest counter
    /// exceeds the threshold.  Called after each `ingest_*` operation.
    fn maybe_sweep_pending(&self) {
        ...
        if count.is_multiple_of(PENDING_SWEEP_THRESHOLD) {
            let pruned = self.sweep_stale_pending(PENDING_TTL);
            if pruned > 0 {
                tracing::debug!(
                    pruned,
                    total_ingests = count,
    ...
    pub fn remove_worker_all_media(&self, instance_id: &str, dp_rank: u32) {
        ...
        // this worker.  Offload entries have no per-worker association so
        // we cannot selectively purge them here; they are bounded by the
        // number of outstanding offloaded blocks.
        {
            let mut state = self.offload_pool_state.write();
            state.pending_pool.retain(|_, entries| {
                entries
